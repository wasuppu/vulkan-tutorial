...
func (app *HelloTriangleApplication) createVertexBuffer() error {
	...
	// bufferSize := uint32(unsafe.Sizeof(Vertex{})) * uint32(len(vertices))
	...
	if vk.CreateBuffer(app.device, &bufferInfo, nil, &vertexBuffer) != vk.Success {
		return fmt.Errorf("failed to create vertex buffer")
	}
	app.vertexBuffer = vertexBuffer

	var memRequirements vk.MemoryRequirements
	vk.GetBufferMemoryRequirements(app.device, app.vertexBuffer, &memRequirements)
	memRequirements.Deref()
	memRequirements.Free()

	memoryTypeIndex, err := findMemoryType(memRequirements.MemoryTypeBits, vk.MemoryPropertyFlags(vk.MemoryPropertyHostVisibleBit|vk.MemoryPropertyHostCoherentBit), app.physicalDevice)
	if err != nil {
		return err
	}
	...
	if vk.AllocateMemory(app.device, &allocInfo, nil, &vertexBufferMemory) != vk.Success {
		return fmt.Errorf("failed to allocate vertex buffer memory")
	}
	app.vertexBufferMemory = vertexBufferMemory

	vk.BindBufferMemory(app.device, app.vertexBuffer, app.vertexBufferMemory, 0)

	var data unsafe.Pointer
	vk.MapMemory(app.device, app.vertexBufferMemory, 0, bufferInfo.Size, 0, &data)
	buf := &bytes.Buffer{}
	binary.Write(buf, chooseByteOrder(), vertices)
	vk.Memcopy(data, buf.Bytes())
	// dataBuffer := unsafe.Slice((*byte)(data), int(bufferSize))
...
func (app *HelloTriangleApplication) recordCommandBuffer(commandBuffer vk.CommandBuffer, imageIndex uint32) error {
	...
	vk.CmdDraw(commandBuffer, uint32(len(vertices)), 1, 0, 0)
