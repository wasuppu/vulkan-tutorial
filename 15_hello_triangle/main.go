...
func (app *HelloTriangleApplication) initVulkan() error {
	...
	if err := app.createCommandBuffer(); err != nil {
		return err
	}
...
func (app *HelloTriangleApplication) createCommandBuffer() error {
	...
	app.commandBuffer = commandBuffers[0]
	return nil
}
...
func (app *HelloTriangleApplication) createSyncObjects() error {
	...
	app.inFlightFence = inFlightFence
...
func (app *HelloTriangleApplication) drawFrame() error {
	vk.WaitForFences(app.device, 1, []vk.Fence{app.inFlightFence}, vk.True, vk.MaxUint64)
	vk.ResetFences(app.device, 1, []vk.Fence{app.inFlightFence})

	var imageIndex uint32
	vk.AcquireNextImage(app.device, app.swapChain, vk.MaxUint64, app.imageAvailableSemaphore, vk.Fence(vk.NullHandle), &imageIndex)

	vk.ResetCommandBuffer(app.commandBuffer, 0)
	app.recordCommandBuffer(app.commandBuffer, imageIndex)

	waitSemaphores := []vk.Semaphore{app.imageAvailableSemaphore}
	waitStages := []vk.PipelineStageFlags{vk.PipelineStageFlags(vk.PipelineStageColorAttachmentOutputBit)}

	commandBuffers := []vk.CommandBuffer{app.commandBuffer}
	signalSemaphores := []vk.Semaphore{app.renderFinishedSemaphore}
	...
	if vk.QueueSubmit(app.graphicsQueue, 1, submitInfos, app.inFlightFence) != vk.Success {
	...
	vk.QueuePresent(app.presentQueue, &presentInfo)
	return nil
}
